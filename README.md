...
- 信号池和观察池 / 持仓观察结构正在拆分。
- 影子候选只用于观察，不直接进入实盘执行。
- Qlib 正在作为后续因子研究与回测容器的候选标准组件评估。
- 系统正在补强“无信号日”的解释能力：市场否决、规则过严、不可买涨停、数据过期、实时行情不可用等。

## 因子研究与回测容器规划

AI Workbench 的公开规划里，会把因子研究与回测能力放在独立容器层，而不是和盘中实盘监测逻辑混在一起。

这一层计划优先评估 Qlib 作为标准研究组件：

- 用 Qlib 承接因子研究、样本切分、模型训练和回测实验。
- 用本地数据治理层把 A 股历史数据整理成研究可用口径。
- 用 AI Workbench 管理研究任务、复盘摘要、策略 postmortem 和候选因子记录。
- 用盘中实时数据链路承接 14:45 信号扫描、观察池和实盘观察。
- 保持研究容器和实盘执行链路隔离，避免回测逻辑直接污染实盘决策。

这部分目前作为公开路线图展示，不代表真实数据、私有因子、策略参数或交易执行逻辑已经开源。
